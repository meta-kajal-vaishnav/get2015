...
public class CountingSort {
	...
	public int[] countingSort(int largestValue) {
		// creating new array of size = largest element and initializing new
		// array to zero
		int arrNew[] = new int[largestValue + 1];
		for (int index = 0; index < arrNew.length; index++)
			arrNew[index] = 0;

		// if item in input array is equal to any index of new array then
		// increase value at that index by one
		for (int item : arr) {
			for (int index = 0; index < arrNew.length; index++) {
				if (item == index)
					arrNew[index] += 1;
