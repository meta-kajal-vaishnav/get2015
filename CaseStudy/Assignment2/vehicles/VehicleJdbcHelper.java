...
public class VehicleJdbcHelper {
	...
	public static String createVehicleTable() throws VehicleManagementException {
		String str = null;
		Connection con = null;
		Statement stmt = null;
		...
		try {
			...
			throw new VehicleManagementException("Cannot create vehicle : " + e);
		} finally {
			/* close connection */
			try {
				if (con != null) {
		...
		try {
			...
			if (countVehicle > 0) {
				if (objVehicle instanceof Car) {
					...
					if (countCar > 0)
						str = "Car inserted Successfully";
					else
						str = "Could not insert Car";
				} else if (objVehicle instanceof Bike) {
					countBike = BikeJdbcHelper
							.insertInBikeTable(((Bike) objVehicle));
					if (countBike > 0)
						str = "Bike inserted Successfully";
					else
						str = "Could not insert Bike";
				}
			} else
				str = "Could not insert vehicle";
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			/* close connection */
	...
	public static String deleteVehicleTable() throws VehicleManagementException {
		String str = null;
		Connection con = null;
		Statement stmt = null;
		ConnectionUtil conUtil = new ConnectionUtil();
		...
		try {
			stmt = con.createStatement();
			stmt.executeUpdate(query);
			str = "Vehicle deleted successfully";
		} catch (SQLException e) {
			throw new VehicleManagementException("Cannot create vehicle : " + e);
		} finally {
			/* close connection */
